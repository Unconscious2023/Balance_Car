...
void PS2_Contorl_Car(void)
{
	...
	else
	{
		...
		{
			g_newcarstate=enps2Fleft; //前左旋 Front left turn
		}
		...
		{ 
			g_newcarstate=enps2Bleft; //后左旋 Back left turn
		}
		else if((g_PS2_LX>150 && g_PS2_LY>150) || (g_PS2_RX>150 && g_PS2_RY>150))
		{
			g_newcarstate=enps2Bright; //后右旋 Back right turn
		}
	...
	switch (g_PS2_KEY)
	{
		...
		//停止 stop
		default:
			if(g_flag == 1) //红绿模式才判断 Red and green mode is used to judge
			{
