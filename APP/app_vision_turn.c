...
float Vision_Min_Run_Speed = 12.0f;
float Vision_Min_Curve_Speed = 6.0f;
...
float VisionTurn_Calc(float gyro_z)
{
    ...
    turn_pwm -= (int)(gyro_z * 0.05f);   // 陀螺阻尼 Kd=0.05, 抑制振荡
...
float VisionSpeed_Target(float base_speed)
{
    ...
    if (g_vision_input.confidence < VISION_MIN_CONFIDENCE) {
        // 倒车逻辑已禁用
        // if ((int16_t)base_speed > 0 && g_vision_frame_count > 0) {
        //     g_vision_target_speed = -(float)(int16_t)base_speed;
